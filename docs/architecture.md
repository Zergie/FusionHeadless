...
## Route and tool packages

`routes/__init__.py` and `mcp/tools/__init__.py` explicitly import each operation.
...
On restart, `extension_state.py` discards all loaded route and tool submodules
...
The fingerprint covers every Python source file under `routes/` and
`mcp/tools/`, including their initializers and helpers, plus `fusion_support.py`.
...
python -m compileall -q FusionHeadless.py adapter.py bridge.py context.py extension_state.py fusion_host.py server.py routes mcp fusion_support.py routing.py fusion_invocation.py versioning.py cli tests
git diff --check
```
